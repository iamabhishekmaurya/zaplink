...
class TraceIdFilterTest
{
    ...
    void doFilterInternal_ShouldExtractTraceIdFromHeader()
        throws ServletException,
        IOException
    {
        // Given
        String incomingTraceId = "incoming-trace-id-123";
        when( request.getHeader( TraceContext.getTraceIdHeader() ) ).thenReturn( incomingTraceId );
        ...
        verify( response ).setHeader( TraceContext.getTraceIdHeader(), incomingTraceId );
        verify( filterChain ).doFilter( request, response );
    }

    @Test @DisplayName("Filter should generate trace ID when not in header")
    void doFilterInternal_ShouldGenerateTraceIdWhenNotInHeader()
        throws ServletException,
        IOException
    {
        // Given
        when( request.getHeader( TraceContext.getTraceIdHeader() ) ).thenReturn( null );
        ...
        verify( response ).setHeader( TraceContext.getTraceIdHeader(), generatedTraceId );
        verify( filterChain ).doFilter( request, response );
    }

    @Test @DisplayName("Filter should generate trace ID when header is empty")
    void doFilterInternal_ShouldGenerateTraceIdWhenHeaderIsEmpty()
        throws ServletException,
        IOException
    {
        // Given
        when( request.getHeader( TraceContext.getTraceIdHeader() ) ).thenReturn( "   " );
    ...
    void doFilterInternal_ShouldTrimWhitespaceFromTraceId()
        throws ServletException,
        IOException
    {
        // Given
        ...
        when( request.getHeader( TraceContext.getTraceIdHeader() ) ).thenReturn( traceIdWithSpaces );
        ...
        verify( response ).setHeader( TraceContext.getTraceIdHeader(), expectedTraceId );
        verify( filterChain ).doFilter( request, response );
    }

    @Test @DisplayName("Filter should clear trace ID in finally block")
    void doFilterInternal_ShouldClearTraceIdInFinallyBlock()
        throws ServletException,
        IOException
    {
        // Given
        when( request.getHeader( TraceContext.getTraceIdHeader() ) ).thenReturn( "test-trace-id" );
        when( request.getMethod() ).thenReturn( "GET" );
        when( request.getRequestURI() ).thenReturn( "/api/test" );
