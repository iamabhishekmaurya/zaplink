...
public class CoreClient {
    ...
    public BioPageDto getBioPage(Long bioPageId) {
        String url = UriComponentsBuilder.fromUriString(coreServiceUrl)
                .path("/api/bio-pages/{id}")
                .buildAndExpand(bioPageId)
                .toUriString();
        
        return restTemplate.getForObject(url, BioPageDto.class);
    }
    ...
    public boolean organizationExists(Long organizationId) {
        String url = UriComponentsBuilder.fromUriString(coreServiceUrl)
                .path("/api/organizations/{id}/exists")
                .buildAndExpand(organizationId)
                .toUriString();
    ...
    public BioPageDto createBioPage(BioPageDto bioPage) {
        String url = UriComponentsBuilder.fromUriString(coreServiceUrl)
                .path("/api/bio-pages")
                .build()
                .toUriString();
        
        return restTemplate.postForObject(url, bioPage, BioPageDto.class);
    }
