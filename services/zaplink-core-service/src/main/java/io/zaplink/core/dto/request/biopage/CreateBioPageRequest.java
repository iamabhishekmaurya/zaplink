 ...
 */
public record CreateBioPageRequest( @NotBlank(message = ErrorConstant.VALIDATION_USERNAME_REQUIRED) @Size(min = 3, max = 50, message = ErrorConstant.VALIDATION_USERNAME_SIZE_RANGE) String username,
                                    ...
                                    @JsonProperty("is_public") Boolean isPublic )
{
    ...
    private void validateUsername( String username )
    {
        ...
        switch ( username )
        {
            case String uname when uname.trim()
                    .isEmpty() -> throw new IllegalArgumentException( ErrorConstant.ERROR_USERNAME_CANNOT_BE_EMPTY );
            case String uname when !uname
                    .matches( QrConstants.USERNAME_PATTERN ) -> throw new IllegalArgumentException( ErrorConstant.ERROR_USERNAME_FORMAT );
            case String uname when uname.startsWith( "-" ) || uname
                    .startsWith( "_" ) -> throw new IllegalArgumentException( ErrorConstant.ERROR_USERNAME_CANNOT_START_WITH_UNDERSCORE_OR_HYPHEN );
            default -> {
                // Valid username
            }
        }
